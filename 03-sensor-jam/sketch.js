...
function draw() {
  ...
  if (port.opened()) {
    ...
    if (sensorVal[0]) {
      ...
      circleSize = lerp(circleSize, val, 0.1);
      rectSize = lerp(rectSize, val/2, 0.1);
      

      let c = map(val, 0, 1023, 0, 255);
      //  fill(c);
       fill(c, 100, 255 - c); // used help from genai
       stroke(255 - c, 100, c);
